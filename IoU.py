...
def bb_intersection_over_union(detected, real):
...
df_detected = pd.read_csv('result/outputs.csv', converters={'output': literal_eval})
c = 0
iou_score = 0
for row in df_detected['image']:
    detected_box = pd.DataFrame({'1': [], '2': [], '3': [], '4': []})
    # coord = df_detected.loc[df_detected['image'] == row]['output'].item().replace('[', '').replace(']', '').split(',')
    # real_box =
    # coord = df_detected.loc[df_detected['image'] == row]['output']
    # print(df_detected.loc[df_detected['image'] == row]['output'], type(df_detected.loc[df_detected['image'] == row]['output']))
    # detected_box = [int(coord[1]), int(coord[2]), int(coord[3]), int(coord[4])]
    # print(detected_box)
    # c += 1
    # if c == 28:
    coord = df_detected.loc[df_detected['image'] == row]['output']
    ...
    for i in range(detected_box.shape[0]):
        ...
        xml_df = xml_get_bnd_boxes(row.split('.')[0] + '.xml')
        all_diff_boxes = []
        for j in range(xml_df.shape[0]):
            all_diff_boxes.append(bb_intersection_over_union(detected_box.loc[i], xml_df.loc[j]))
